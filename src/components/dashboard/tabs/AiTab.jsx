        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            <h4 className="text-lg font-black text-slate-950 flex items-center gap-2">
              ...
              <span>{t.iaTokenUsage}</span>
              {renderInfoTooltip && renderInfoTooltip(
                "tokenMetricsExplanation",
                <div className="space-y-2 text-left">
                  <p className="font-extrabold text-white text-xs flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-blue-400 shrink-0"></span>
                    <span>{lang === "es" ? "¿Qué son los tokens?" : "What are tokens?"}</span>
                  </p>
                  <p className="text-[11px] text-slate-300 leading-relaxed">
                    {lang === "es"
                      ? "Son las unidades básicas con las que la IA procesa y genera texto (1 token equivale aprox. a 4 caracteres o 0.75 palabras)."
                      : "They are the basic units AI uses to process and generate text (1 token ≈ 4 characters or 0.75 words)."}
                  </p>
                  <div className="pt-1.5 border-t border-slate-800/90 space-y-1 text-[10.5px]">
                    <p className="text-slate-300">
                      <strong className="text-blue-300 font-bold">{lang === "es" ? "• Entrada (Prompt):" : "• Input (Prompt):"}</strong>{" "}
                      {lang === "es" ? "Preguntas de visitantes e información enviada a la IA." : "Visitor queries and context sent to the AI."}
                    </p>
                    <p className="text-slate-300">
                      <strong className="text-emerald-300 font-bold">{lang === "es" ? "• Salida (Completion):" : "• Output (Completion):"}</strong>{" "}
                      {lang === "es" ? "Respuestas redactadas por el chatbot para el cliente." : "Answers written by the chatbot for the client."}
                    </p>
                  </div>
                </div>,
                "shift-left-mobile",
            ...
            <div className="lg:col-span-7 bg-white border border-slate-200/90 rounded-2xl p-5 shadow-xs">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl text-center">
                  <span className="text-slate-400 text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1 mb-1">
                    <span>{t.iaPromptTokens}</span>
                    {renderInfoTooltip && renderInfoTooltip(
                      "promptTokensHelp",
                      lang === "es"
                        ? "Tokens de entrada: preguntas del usuario, contexto de la empresa e historial enviado al modelo de IA."
                        : "Input tokens: visitor queries, business knowledge, and chat context sent to the AI.",
                      "center"
                    )}
                  </span>
                ...
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl text-center">
                  <span className="text-slate-400 text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1 mb-1">
                    <span>{t.iaCompletionTokens}</span>
                    {renderInfoTooltip && renderInfoTooltip(
                      "completionTokensHelp",
                      lang === "es"
                        ? "Tokens de salida: contenido y respuestas redactadas por el chatbot de IA para el usuario."
                        : "Output tokens: text and responses generated by the AI chatbot for the user.",
                      "center"
                    )}
                  </span>
                ...
                <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl text-center">
                  <span className="text-slate-500 text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1 mb-1">
                    <span>{t.iaTotalTokens}</span>
                    {renderInfoTooltip && renderInfoTooltip(
                      "totalTokensHelp",
                      lang === "es"
                        ? "Consumo total de tokens (Entrada + Salida) facturado o registrado durante el periodo."
                        : "Total tokens consumed (Prompt + Completion) during the period.",
                      "shift-left-mobile"
                    )}
                  </span>
