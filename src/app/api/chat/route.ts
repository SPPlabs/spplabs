    ...
    if (shouldRecordConversation) {
      ...
      if (!activeConversation) {
        try {
          ...
          // Trigger tenant notification if enabled in preferences
          import("@/lib/userNotifications").then(({ sendTenantNotification }) => {
            sendTenantNotification({
              websiteId: website.id,
              type: "ai_chat",
              title: "Nueva conversación con tu Chatbot IA",
              message: "Un visitante ha iniciado una nueva conversación con tu asistente virtual.",
              ctaText: "Ver Conversación en el Dashboard",
              ctaUrl: "/dashboard?tab=ai",
            });
          }).catch(e => logger.error("Failed to trigger tenant AI chat notification:", e));
        } catch (e: unknown) {
