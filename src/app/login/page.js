...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col justify-center items-center relative overflow-hidden px-4 selection:bg-brand-blue selection:text-white">
      ...
      <div className="absolute top-[-20%] left-[-20%] w-[60%] h-[60%] rounded-full bg-brand-blue/10 blur-[120px] -z-10 animate-pulse"></div>
      <div className="absolute bottom-[-20%] right-[-20%] w-[60%] h-[60%] rounded-full bg-brand-green/10 blur-[120px] -z-10"></div>
      ...
      <div className="w-full max-w-md bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl relative">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Access your client dashboard</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full h-12 bg-white text-black hover:bg-brand-blue hover:text-white rounded-xl text-sm font-bold shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center cursor-pointer disabled:opacity-50"
          >
            {loading ? (
