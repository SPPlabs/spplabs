  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden bg-slate-50 text-slate-900 flex flex-col justify-center items-center py-6 sm:py-12 px-4 selection:bg-brand-blue selection:text-white">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none -z-10">
        <div className="absolute top-[-20%] right-[-20%] w-[60%] h-[60%] rounded-full bg-brand-green/5 blur-[120px] animate-pulse"></div>
        <div className="absolute bottom-[-20%] left-[-20%] w-[60%] h-[60%] rounded-full bg-brand-blue/5 blur-[120px]"></div>
      </div>
      ...
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xl relative">
        ...
        <form onSubmit={handleSignup} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl pl-4 pr-12 text-sm font-medium text-slate-900 placeholder-slate-400 focus:outline-none focus:border-brand-blue focus:bg-white transition-all font-mono"
              />
              <div className="absolute right-3.5 top-3.5 text-slate-400 pointer-events-none">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          {/* Tick boxes over activate account button */}
          <div className="space-y-3 pt-1 pb-1 text-slate-700">
            <label className="flex items-start gap-2.5 text-xs cursor-pointer select-none">
              <input
                ...
                className="mt-0.5 w-4 h-4 text-brand-green rounded border-slate-300 focus:ring-brand-green cursor-pointer shrink-0"
              />
              <span>
                {lang === "es" ? "He aceptado y leído los " : "I have read and accepted the "}
                <a href="/terminos-y-condiciones" target="_blank" className="font-semibold text-brand-blue hover:underline">
            ...
            <label className="flex items-start gap-2.5 text-xs cursor-pointer select-none">
              ...
              />
              <span>
                {lang === "es" ? "He aceptado y leído la " : "I have read and accepted the "}
                <a href="/politica-de-privacidad" target="_blank" className="font-semibold text-brand-blue hover:underline">
                  {lang === "es" ? "política de privacidad" : "privacy policy"}
                </a>
                {lang === "es" ? " y la " : " and the "}
                <a href="/politica-de-cookies" target="_blank" className="font-semibold text-brand-blue hover:underline">
