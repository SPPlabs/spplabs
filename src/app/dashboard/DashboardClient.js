...
export default function DashboardClient({
  ...
  const loadNotificationPreferences = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && json.data) {
        ...
        setNotifContacts(d.notifyContacts ?? true);
        setNotifBookings(d.notifyBookings ?? true);
        setNotifAiChats(d.notifyAiChats ?? false);
        setNotifSppAnnouncements(d.notifySppAnnouncements ?? true);
        setNotifMonthlyReport(d.notifyMonthlyReport ?? true);
        setNotifGeneralSummary(d.notifyGeneralSummary ?? true);
      }
    } catch (err) {
  ...
  }, [showSettingsModal, loadNotificationPreferences]);

  // Open settings modal if URL has ?openSettings=notifications
  useEffect(() => {
    if (searchParams?.get("openSettings") === "notifications") {
    ...
    try {
      const res = await fetch("/api/user/notification-preferences", {
        ...
        body: JSON.stringify({
          ...
          notifyAiChats: notifAiChats,
          notifySppAnnouncements: notifSppAnnouncements,
          notifyMonthlyReport: notifMonthlyReport,
          notifyGeneralSummary: notifGeneralSummary,
        }),
      });
  ...
  const handleSendTestNotification = async () => {
    if (!notifEnabled || !notifEmail.trim()) {
      setTestNotifFeedback({
        type: "error",
        message: lang === "es" ? "Primero activa las notificaciones y escribe tu correo." : "First enable notifications and enter your email.",
      });
      return;
    }
    ...
    try {
      // Autosave current preferences before testing
      ...
      const res = await fetch("/api/user/notification-preferences/test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ domain: currentWebsite?.domain }),
      });
      const data = await res.json();
      if (res.ok) {
        setTestNotifFeedback({
          type: "success",
          message: lang === "es" ? `¡Correo de prueba enviado a ${notifEmail}!` : `Test email sent to ${notifEmail}!`,
        });
        setTimeout(() => setTestNotifFeedback(null), 6000);
      } else {
        setTestNotifFeedback({
          type: "error",
          message: data.message || (lang === "es" ? "Error al enviar correo de prueba" : "Error sending test email"),
        });
      }
    } catch {
      setTestNotifFeedback({
        type: "error",
        message: lang === "es" ? "Error de conexión al enviar prueba" : "Connection error sending test",
      });
    } finally {
  ...
  const handleUpdateChatbotKnowledge = async (e) => {
    e.preventDefault();
    if (chatbotContent.length > 50000) {
      alert("El contenido de la base de conocimiento no puede superar los 50,000 caracteres.");
      return;
    }
    setIaSaving(true);
    setIaSaved(false);
        ...
        >
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-scale-up flex flex-col max-h-[90vh] sm:max-h-[85vh]">
            ...
            <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-6">
              ...
              <div id="settings-notifications-section" className="border-t border-slate-150 pt-5 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-xl bg-sky-500/10 border border-sky-200 text-sky-600 flex items-center justify-center shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        {lang === "es"
                          ? "Recibe alertas en tu correo cuando ocurra nueva actividad en tu web"
                          : "Receive alerts in your inbox when new activity occurs on your website"}
                      </p>
                    </div>
                  </div>
                ...
                {notifEnabled ? (
                  <div className="p-4 bg-slate-50 border border-slate-200/90 rounded-2xl space-y-4 animate-fade-in mt-3">
                    {/* Destination email input */}
                    <div>
                      <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-600 mb-1">
                        {lang === "es" ? "Correo para recibir notificaciones *" : "Notification destination email *"}
                      </label>
                      <input
                        type="email"
                        required
                        ...
                        className="w-full h-10 bg-white border border-slate-200 rounded-xl px-3 text-xs font-semibold text-slate-900 placeholder-slate-400 focus:outline-none focus:border-brand-blue transition-all"
                      />
                      <p className="text-[10.5px] text-slate-400 mt-1">
                        {lang === "es"
                          ? "Aquí recibirás las alertas de actividad relevante de tu web."
                          : "This address will receive alert emails when relevant activity occurs."}
                      </p>
                    </div>
                    ...
                    <div>
                      <span className="block text-[11px] font-bold uppercase tracking-wider text-slate-600 mb-2">
                        {lang === "es" ? "Tipos de notificaciones a recibir:" : "Notification types to receive:"}
                      </span>
                      <div className="space-y-2">
                        {/* 1. Contact Forms */}
                        <label className="flex items-start gap-2.5 p-2.5 bg-white border border-slate-200/80 rounded-xl cursor-pointer hover:border-slate-300 transition-colors">
                          <input
                            type="checkbox"
                            checked={notifContacts}
                            onChange={(e) => setNotifContacts(e.target.checked)}
                          ...
                          <div className="text-xs">
                            <span className="font-bold text-slate-800 block">
                              {lang === "es" ? "Formularios de contacto" : "Contact forms"}
                            </span>
                            <span className="text-slate-500 text-[11px] block mt-0.5">
                              {lang === "es"
                                ? "Avisos inmediatos cuando un visitante te escriba desde el formulario web."
                                : "Immediate alerts when a visitor writes through the website contact form."}
                            </span>
                          </div>
                        </label>
                        ...
                        <label className="flex items-start gap-2.5 p-2.5 bg-white border border-slate-200/80 rounded-xl cursor-pointer hover:border-slate-300 transition-colors">
                          ...
                          <div className="text-xs">
                            <span className="font-bold text-slate-800 block">
                              {lang === "es" ? "Reservas y Citas" : "Bookings & Appointments"}
                            </span>
                            <span className="text-slate-500 text-[11px] block mt-0.5">
                              {lang === "es"
                                ? "Alertas cuando un cliente solicite una cita en tu calendario web."
                                : "Alerts when a client books a slot in your web calendar."}
                            </span>
                          </div>
                        </label>
                        ...
                        <label className="flex items-start gap-2.5 p-2.5 bg-white border border-slate-200/80 rounded-xl cursor-pointer hover:border-slate-300 transition-colors">
                          <input
                            type="checkbox"
                            checked={notifAiChats}
                            onChange={(e) => setNotifAiChats(e.target.checked)}
                            className="mt-0.5 w-4 h-4 rounded text-brand-blue border-slate-300 focus:ring-brand-blue cursor-pointer accent-black"
                          />
                          <div className="text-xs">
                            <span className="font-bold text-slate-800 block">
                              {lang === "es" ? "Conversaciones con Chatbot IA" : "AI Chatbot conversations"}
                            </span>
                            <span className="text-slate-500 text-[11px] block mt-0.5">
                              {lang === "es"
                                ? "Notificaciones cuando un visitante interactúe con el asistente de IA."
                                : "Notifications when a visitor chats with the AI assistant."}
                            </span>
                          </div>
                        </label>

                        {/* 4. SPP Labs Announcements */}
                        <label className="flex items-start gap-2.5 p-2.5 bg-white border border-slate-200/80 rounded-xl cursor-pointer hover:border-slate-300 transition-colors">
                          <input
                            type="checkbox"
                            checked={notifSppAnnouncements}
                            onChange={(e) => setNotifSppAnnouncements(e.target.checked)}
                            className="mt-0.5 w-4 h-4 rounded text-brand-blue border-slate-300 focus:ring-brand-blue cursor-pointer accent-black"
                          />
                          <div className="text-xs">
                            <span className="font-bold text-slate-800 block">
                              {lang === "es" ? "Notificaciones y Avisos de SPP Labs" : "SPP Labs announcements & alerts"}
                            </span>
                            <span className="text-slate-500 text-[11px] block mt-0.5">
                              {lang === "es"
                                ? "Avisos y comunicados oficiales emitidos por los administradores de SPP Labs."
                                : "Official alerts and announcements from SPP Labs administrators."}
                            </span>
                          </div>
                        </label>
                        ...
                        <label className="flex items-start gap-2.5 p-2.5 bg-white border border-slate-200/80 rounded-xl cursor-pointer hover:border-slate-300 transition-colors">
                          <input
                            type="checkbox"
                            checked={notifMonthlyReport}
                            onChange={(e) => setNotifMonthlyReport(e.target.checked)}
                            className="mt-0.5 w-4 h-4 rounded text-brand-blue border-slate-300 focus:ring-brand-blue cursor-pointer accent-black"
                          />
                          <div className="text-xs">
                            <span className="font-bold text-slate-800 block">
                              {lang === "es" ? "Informe Mensual de Rendimiento" : "Monthly Performance Report"}
                            </span>
                            <span className="text-slate-500 text-[11px] block mt-0.5">
                              {lang === "es"
                                ? "Aviso inmediato cuando el balance analítico del mes esté generado y listo."
                                : "Alert when the monthly analytics report is generated and ready."}
                            </span>
                          </div>
                        </label>
                        ...
                        <label className="flex items-start gap-2.5 p-2.5 bg-white border border-slate-200/80 rounded-xl cursor-pointer hover:border-slate-300 transition-colors">
                          <input
                            type="checkbox"
                            checked={notifGeneralSummary}
                            onChange={(e) => setNotifGeneralSummary(e.target.checked)}
                            className="mt-0.5 w-4 h-4 rounded text-brand-blue border-slate-300 focus:ring-brand-blue cursor-pointer accent-black"
                          />
                          <div className="text-xs">
                            <span className="font-bold text-slate-800 block">
                              {lang === "es" ? "Resumen General de Actividad" : "General Activity Summary"}
                            </span>
                            <span className="text-slate-500 text-[11px] block mt-0.5">
                              {lang === "es"
                                ? "Avisos periódicos o agrupados cuando se acumulen varias novedades pendientes."
                                : "Grouped alerts when multiple pending updates accumulate."}
                            </span>
                          </div>
                        </label>
                      </div>
                    ...
                    {/* Teaser note about privacy */}
                    <div className="p-3 bg-blue-50/60 border border-blue-200/70 rounded-xl text-[11px] text-blue-900 leading-relaxed">
                      💡 <strong>{lang === "es" ? "Aviso de privacidad:" : "Privacy notice:"}</strong>{" "}
                      {lang === "es"
                        ? "Los correos de alerta no revelan los datos sensibles en el mensaje; contienen un enlace directo para que puedas consultar y responder a todo de forma segura desde tu panel de SPP Labs."
                        : "Alert emails do not expose sensitive content; they include a secure link to view and reply directly from your SPP Labs dashboard."}
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-200 flex-wrap">
                      <button
                        ...
                        disabled={isSendingTestNotif || isSavingNotif || !notifEmail.trim()}
                        className="px-3 py-2 bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-all disabled:opacity-50 cursor-pointer flex items-center gap-1.5 shadow-2xs"
                        title={lang === "es" ? "Enviar un email de prueba para comprobar el diseño" : "Send a test email to verify design"}
                      >
                        {isSendingTestNotif ? (
                        ...
                        <span>{lang === "es" ? "Probar Notificación" : "Test Notification"}</span>
                      </button>
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-slate-900 hover:bg-black text-white text-xs font-bold rounded-xl transition-all disabled:opacity-50 cursor-pointer flex items-center gap-1.5 shadow-xs"
                      >
                        {isSavingNotif ? (
                          <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
                        ) : (
                        ...
                        <span>{lang === "es" ? "Guardar Preferencias" : "Save Preferences"}</span>
                      </button>
                    </div>
                  ...
                  <p className="text-xs text-slate-400 italic bg-slate-50 border border-slate-150 rounded-xl p-3">
                    {lang === "es"
                      ? "Las notificaciones por email están desactivadas. Activa el interruptor para elegir qué alertas deseas recibir y en qué correo."
                      : "Email notifications are turned off. Toggle the switch to choose which alerts you want to receive and at which email address."}
                  </p>
                )}
              </div>
